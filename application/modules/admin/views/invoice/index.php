<div class="header smaller lighter">
    <h4><i class="fa fa-newspaper-o"></i> Invoice Information
        <?php if (!empty($_SESSION) && ($_SESSION['ROLE'] != "SUPER_ADMIN")) { ?>
            <span class="pull-right">
            <a href="<?php echo base_url(); ?>admin/invoice/add/" class="btn btn-info btn-bold btn-sm"><i class="fa fa-plus-circle"></i> Add</a>
        </span>
        <?php } ?>
    </h4>
</div>
...
if ($_SESSION['ROLE'] == 'SUPER_ADMIN') {
    ...
    <form class="form-inline text-center" id="frm" method="get">
        ...
        <a href="<?php echo base_url() ?>admin/invoice/" class="btn btn-danger btn-xs">Refresh</a>
    </form>
    <div class="space-6"></div>
    <?php
} ?>
...
<table class="table table-bordered table-hover">
    ...
    if (!empty($invoices)) {
        ...
        foreach ($invoices as $invoice) { ?>
            <tr>
                <td><?php echo $i ?></td>
                <td><?php echo !empty($invoice['invoice']) ? $invoice['invoice'] : ''; ?></td>
                ...
                <td>
                    <a href="<?php echo get_role_based_link() ?>/invoice/view/<?php echo $invoice['pk_id'] ?>" data-toggle="tooltip" title="View" class="btn btn-warning btn-xs"><i class="fa fa-eye"></i> </a>
                    <a href="<?php echo get_role_based_link() ?>/invoice/edit/<?php echo $invoice['pk_id'] ?>" data-toggle="tooltip" title="Edit" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> </a>
                    <!--<a href="#" class="btn btn-xs btn-success"><i data-toggle="tooltip" title="Click to Send Message" class="fa fa-envelope-o"></i></a>-->
                </td>
            </tr>
            <?php $i++;
        }
